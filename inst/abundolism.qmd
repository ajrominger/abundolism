...
where $x_i$ is mean abundance of lineage $i$. This model means that we allow the $\alpha$ parameter of the beta geometric distribution to change with lineage abundance but we assume the $\beta$ parameter is fixed across lineages. In this model, the expected value of $y_i$ given $x_i$ is 
...
```{r}
curve(dbeta(x, 0.1, 1), log = "y")
curve(dbeta(x, 0.1, 10), col = "red", add = TRUE)


plot(dyules(1:1000, 0.01), log = "y")
points(dyules(1:1000, 1), col = "red")
```


Hold up....could model it as Yule-Simons distribution which would be a geometric compounded with a beta distribution to account for different times of arrival
...
- slow down of progress toward specation is relative to pop size: `xi(r) / x(receiving_pop);`
