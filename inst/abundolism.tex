...
incipient speciation \(\nu\) is 0.01 per individual per 10,000 years.
...
$\lambda$ & (\texttt{la}) & 0.1 & 4.00 & per capita local birth rate\\
$\mu$ & (\texttt{mu}) & 0.1 & 4.00 & per capita local death rate\\
$\gamma$ & (\texttt{g}) & 0.1 & 1.00 & dispersal rate from the global source pool to one local population\\
$m_\text{prop}$ & (\texttt{m\_prop}) & 0.0 & 0.10 & proportional dispersal rate between local populations relative to global rate\\
$\nu$ & (\texttt{nu}) & 0.0 & 0.01 & per capita incipient speciation rate\\
\addlinespace
$\tau$ & (\texttt{tau}) & 20.0 & 200.00 & wait time to full speciation in the absence of dispersal\\
$\xi$ & (\texttt{xi}) & 10.0 & 100.00 & amount each dispersal event sets back the progression toward speciation\\
$n_p$ & (\texttt{np}) & 2.0 & 2.00 & number of local populations\\
$n_\text{step}$ & (\texttt{nstep}) & 10,000.0 & 10,000.00 & number of simulation iterations\\
\bottomrule
\end{tabular}
...
\] where \(y\) is number of species in a lineage and \(1 - \psi(t)\) is
the probability of at least one bifurcation event occurring prior to
time \(t\) and decedents of that bifurcation event being extant at time
\(t\) (\textbf{nee?}). \(\psi(t)\) depends on rates of speciation and
extinction in addition to time \(t\).
...
So our final, quadratic model relating genus richness to genus abundance
will be
...
where \(x_i\) is mean abundance of lineage \(i\). This model means that
we allow the \(\alpha\) parameter of the beta geometric distribution to
change with lineage abundance but we assume the \(\beta\) parameter is
fixed across lineages. In this model, the expected value of \(y_i\)
given \(x_i\) is

\[
\mathbb{E}[y_i \mid x_i] = \frac{\alpha_i + \beta}{\alpha_i}
\]
...
\item
  slow down of progress toward specation is relative to pop size:
  \texttt{xi(r)\ /\ x(receiving\_pop);}
\end{itemize}
